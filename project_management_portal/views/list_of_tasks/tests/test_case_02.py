...
class TestCase01ListOfTasksAPITestCase(CustomTestUtils):
    app_name = APP_NAME
    ...
    def setupUser(self, username, password):
        super(TestCase01ListOfTasksAPITestCase, self).setupUser(
            username=username, password=password
        )
        project = ProjectFactory()
        state = StateFactory()
        TaskFactory.create_batch(5, project=project, state=state)
